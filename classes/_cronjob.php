...
$restaurants_json = file_get_contents(ROOT_FOLDER . "config/restaurants.json");
...
foreach($filtered_matches as $key => $value) {
  if(preg_match('/(<div class="restaurant_info">)(.*?)(<\/div> <!--\/\.left-->)/mi', $value, $found_match)) {
    if($restaurants_obj->restaurants->{$key}) {
      if(!file_exists(ROOT_FOLDER . "images/restaurants/" . $key . ".png")) {
...
if($cache_valid) {
  if($cache_obj->content_hash !== $restaurants_obj->content_hash) {
    file_put_contents(ROOT_FOLDER . "cache.json", $json_encoded);
    echo "Cache updated (#2)!";
  } else echo "Cache is up-to-date!";
} else {
  file_put_contents(ROOT_FOLDER . "cache.json", $json_encoded);
  echo "Cache updated (#1)!";
}
